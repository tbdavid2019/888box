...
一款專業級、全方位的媒體與文件託管解決方案。採用全新的 **Bento Grid** 門戶設計，將「圖片」、「影片」、「音訊」與「文件」完美整合，並具備強大的安全性、分析與舉報系統。支援 AWS S3 等多種儲存後端，具備自動提取 MetaData、Podcast RSS 同步、受控的文字／EPUB 線上閱讀及點擊次數追蹤等功能。
...
- **統一置頂導覽 (Sticky Header)**：首頁、上傳中心與分享頁使用一致的置頂導覽結構；分享頁保留麵包屑、公開資源標籤、語言切換與上傳入口，並在捲動時固定於視窗頂部。
...
- **RFC 8288 發現連結**：首頁 Response Header 具備 RFC 8288 `Link` 屬性，引導 Agent 自動探測 sitemap、api-catalog、llms.txt 與技能頁。
